...
Vision towns confirmed so far: in Connecticut, Pomfret, Woodstock, Thompson, Union, Willington, Hampton, Stafford, Tolland, Coventry, Brooklyn, Plainfield, Canterbury, Andover, Bolton, Sterling, Manchester, East Windsor, Granby, Glastonbury, Enfield, Somers, South Windsor, Norwich, Griswold, Lisbon, Sprague, East Lyme, Preston, Waterford, Old Lyme and Salem; in Massachusetts, Sturbridge, Dudley, Southbridge, Wales, Holland, Palmer and Ware; in Rhode Island, Foster, Exeter, Hopkinton and Richmond. Village names in notices (Broad Brook, North Grosvenordale, Moosup, Jewett City and others) are mapped to their towns. Ashford itself (propertyrecordcards.com), Eastford, Killingly, Putnam, Mansfield, Chaplin, Scotland, Windham, Cromwell, Hartford and others use other vendors. The town-to-slug table is `VGSI_TOWNS` in `lib/enrich/vgsi.js`; add a town after confirming `https://gis.vgsi.com/<town>ct/` loads. Each run reads at most `COLLECTOR_MAX_CARDS` cards (default 80); pages are cached, so repeat runs re-read only what changed.
